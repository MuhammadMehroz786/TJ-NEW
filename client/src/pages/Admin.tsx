...
interface SupportEmailRow {
  ...
  classifiedAction: "auto_reply" | "skipped";
  skippedReason: string | null;
  receivedAt: string;
  sentAt: string | null;
}

interface SupportEmailDetail extends SupportEmailRow {
  imapUid: number;
...
export function Admin() {
  ...
  // Emails tab
  const [emails, setEmails] = useState<SupportEmailRow[]>([]);
  const [emailTotal, setEmailTotal] = useState(0);
  const [emailSearch, setEmailSearch] = useState("");
  const [emailActionFilter, setEmailActionFilter] = useState<"all" | "auto_reply" | "skipped">("all");
  const [emailPage, setEmailPage] = useState(1);
  const [emailsLoading, setEmailsLoading] = useState(false);
  const [emailDetail, setEmailDetail] = useState<SupportEmailDetail | null>(null);
  ...
  const loadLeads = useCallback(() => {
    ...
    const params: Record<string, string | number> = { page: leadPage, pageSize: LEAD_PAGE_SIZE };
    if (leadSearch) params.search = leadSearch;
    api.get("/admin/whatsapp-leads", { params })
      .then((r) => { setLeads(r.data.data); setLeadTotal(r.data.total); })
  ...
  const openLeadDetail = async (id: string) => {
    ...
    try {
      const r = await api.get(`/admin/whatsapp-leads/${id}`);
      setLeadDetail(r.data.data);
    } catch {
  ...
  const loadEmails = useCallback(() => {
    ...
    const params: Record<string, string | number> = { page: emailPage, pageSize: EMAIL_PAGE_SIZE };
    if (emailSearch) params.search = emailSearch;
    if (emailActionFilter !== "all") params.action = emailActionFilter;
    api.get("/admin/support-emails", { params })
      .then((r) => { setEmails(r.data.data); setEmailTotal(r.data.total); })
      .catch(() => toast.error("Failed to load emails"))
      .finally(() => setEmailsLoading(false));
  }, [emailPage, emailSearch, emailActionFilter]);
  useEffect(() => { if (activeTab === "emails") loadEmails(); }, [activeTab, loadEmails]);

  const openEmailDetail = async (id: string) => {
    setEmailDetailLoading(true);
    try {
      const r = await api.get(`/admin/support-emails/${id}`);
      setEmailDetail(r.data.data);
    } catch {
  ...
  return (
    <div className="space-y-6">
        ...
        <Card className="border-slate-200">
          <CardContent className="p-0">
            <div className="p-4 border-b border-slate-100 flex items-center gap-3">
              ...
              <span className="text-sm text-slate-500 ml-auto">{leadTotal.toLocaleString()} total</span>
            </div>
            {leadsLoading ? (
              <div className="p-8 text-center text-sm text-slate-400">Loading...</div>
            ) : leads.length === 0 ? (
              <div className="p-8 text-center text-sm text-slate-400">No leads match</div>
            ) : (
              <div className="overflow-x-auto">
                <table className="w-full text-sm">
                  <thead className="bg-slate-50 border-b border-slate-100">
                    <tr className="text-left text-slate-500 text-xs uppercase tracking-wide">
                      ...
                      <th className="px-4 py-2.5 font-semibold">Msgs</th>
                      <th className="px-4 py-2.5 font-semibold">First seen</th>
                      <th className="px-4 py-2.5 font-semibold">Last msg</th>
                      <th className="px-4 py-2.5"></th>
                    </tr>
                  </thead>
                  <tbody>
                    {leads.map((l) => (
                      <tr key={l.id} className="border-b border-slate-50 hover:bg-slate-50/60">
                        <td className="px-4 py-3 font-medium text-slate-900">
                          {l.profileName || <span className="text-slate-400">—</span>}
                          {l.linkedUser && (
                            <Badge variant="secondary" className="ml-2 bg-teal-100 text-teal-700">user</Badge>
                          )}
                        </td>
                        <td className="px-4 py-3 text-slate-600 font-mono text-xs">{l.phoneNumber}</td>
                        <td className="px-4 py-3">
                          <Badge variant="secondary" className="bg-slate-100 text-slate-700">{l.state}</Badge>
                        </td>
                        <td className="px-4 py-3 text-slate-500 text-xs uppercase">{l.language}</td>
                        <td className="px-4 py-3 text-slate-600">{l.messageCount}</td>
                        <td className="px-4 py-3 text-slate-500 text-xs">{new Date(l.firstSeenAt).toLocaleDateString()}</td>
                        <td className="px-4 py-3 text-slate-500 text-xs">{new Date(l.lastMessageAt).toLocaleString()}</td>
                        <td className="px-4 py-3 text-right">
                          <Button variant="ghost" size="sm" onClick={() => openLeadDetail(l.id)} className="h-7 px-2">
            ...
            <div className="p-3 border-t border-slate-100 flex items-center justify-between text-xs text-slate-500">
              <span>Page {leadPage} of {Math.max(1, Math.ceil(leadTotal / LEAD_PAGE_SIZE))}</span>
              <div className="flex items-center gap-2">
                <Button variant="outline" size="sm" disabled={leadPage === 1} onClick={() => setLeadPage(leadPage - 1)}>Previous</Button>
                <Button variant="outline" size="sm" disabled={leadPage >= Math.ceil(leadTotal / LEAD_PAGE_SIZE)} onClick={() => setLeadPage(leadPage + 1)}>Next</Button>
              </div>
            </div>
          </CardContent>
        </Card>
        ...
        <Card className="border-slate-200">
          <CardContent className="p-0">
            <div className="p-4 border-b border-slate-100 flex items-center gap-3">
              ...
              <Select value={emailActionFilter} onValueChange={(v) => { setEmailActionFilter(v as typeof emailActionFilter); setEmailPage(1); }}>
                <SelectTrigger className="w-[160px]"><SelectValue /></SelectTrigger>
                <SelectContent>
                  <SelectItem value="all">All</SelectItem>
                  <SelectItem value="auto_reply">Auto-reply drafted</SelectItem>
                  <SelectItem value="skipped">Skipped</SelectItem>
                </SelectContent>
              </Select>
              <span className="text-sm text-slate-500 ml-auto">{emailTotal.toLocaleString()} total</span>
            </div>
            {emailsLoading ? (
              <div className="p-8 text-center text-sm text-slate-400">Loading...</div>
            ) : emails.length === 0 ? (
              <div className="p-8 text-center text-sm text-slate-400">No emails match</div>
            ) : (
              <div className="overflow-x-auto">
                <table className="w-full text-sm">
                  <thead className="bg-slate-50 border-b border-slate-100">
                    ...
                    {emails.map((e) => (
                      <tr key={e.id} className="border-b border-slate-50 hover:bg-slate-50/60">
                        <td className="px-4 py-3 max-w-[220px]">
                          <p className="font-medium text-slate-900 truncate">{e.fromName || e.fromAddress.split("@")[0]}</p>
                          <p className="text-xs text-slate-500 truncate font-mono">{e.fromAddress}</p>
                        </td>
                        <td className="px-4 py-3 text-slate-700 max-w-[280px] truncate">{e.subject || <span className="text-slate-400">—</span>}</td>
                        <td className="px-4 py-3 text-slate-500 text-xs uppercase">{e.language || "—"}</td>
                        <td className="px-4 py-3">
                          {e.classifiedAction === "auto_reply" ? (
                            <Badge variant="secondary" className="bg-teal-100 text-teal-700">auto-reply</Badge>
                          ) : (
                            <Badge variant="secondary" className="bg-slate-100 text-slate-700" title={e.skippedReason || undefined}>
                              skipped
                            </Badge>
                          )}
                        </td>
                        <td className="px-4 py-3 text-xs text-slate-500">
                          {e.sentAt ? new Date(e.sentAt).toLocaleString() : <span className="text-slate-400">—</span>}
                        </td>
                        <td className="px-4 py-3 text-slate-500 text-xs">{new Date(e.receivedAt).toLocaleString()}</td>
                        <td className="px-4 py-3 text-right">
                          <Button variant="ghost" size="sm" onClick={() => openEmailDetail(e.id)} className="h-7 px-2">
                            <ArrowUpRight className="h-3.5 w-3.5" />
                          </Button>
                        </td>
                      </tr>
            ...
            <div className="p-3 border-t border-slate-100 flex items-center justify-between text-xs text-slate-500">
              <span>Page {emailPage} of {Math.max(1, Math.ceil(emailTotal / EMAIL_PAGE_SIZE))}</span>
              <div className="flex items-center gap-2">
                <Button variant="outline" size="sm" disabled={emailPage === 1} onClick={() => setEmailPage(emailPage - 1)}>Previous</Button>
                <Button variant="outline" size="sm" disabled={emailPage >= Math.ceil(emailTotal / EMAIL_PAGE_SIZE)} onClick={() => setEmailPage(emailPage + 1)}>Next</Button>
              </div>
            </div>
          </CardContent>
        </Card>
      ...
      <Dialog open={!!leadDetail || leadDetailLoading} onOpenChange={(open) => { if (!open) setLeadDetail(null); }}>
        <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto">
          ...
          {leadDetailLoading || !leadDetail ? (
            ...
            <div className="space-y-5">
              <div>
                <p className="text-xs font-semibold text-slate-400 uppercase tracking-wide mb-2">Identity</p>
                <div className="grid grid-cols-2 gap-3 text-sm">
                  <div><span className="text-slate-500">Profile name</span><p className="font-medium text-slate-900">{leadDetail.profileName || "—"}</p></div>
                  <div><span className="text-slate-500">Phone</span><p className="font-mono text-slate-900">{leadDetail.phoneNumber}</p></div>
                  <div><span className="text-slate-500">State</span><p className="text-slate-900">{leadDetail.state}</p></div>
                  <div><span className="text-slate-500">Language</span><p className="text-slate-900 uppercase">{leadDetail.language}</p></div>
                  <div><span className="text-slate-500">First seen</span><p className="text-slate-900">{new Date(leadDetail.firstSeenAt).toLocaleString()}</p></div>
                  <div><span className="text-slate-500">Last message</span><p className="text-slate-900">{new Date(leadDetail.lastMessageAt).toLocaleString()}</p></div>
                  <div><span className="text-slate-500">Credits</span><p className="text-slate-900">{leadDetail.creditsUsed}/{leadDetail.creditsLimit}</p></div>
                  <div>
                    <span className="text-slate-500">Linked user</span>
                    <p className="text-slate-900">{leadDetail.user ? `${leadDetail.user.name} (${leadDetail.user.email})` : <span className="text-slate-400">guest</span>}</p>
                  </div>
                </div>
              </div>
              <div>
                <p className="text-xs font-semibold text-slate-400 uppercase tracking-wide mb-2">Conversation ({leadDetail.messages.length})</p>
                {leadDetail.messages.length === 0 ? (
                  <p className="text-sm text-slate-400">No messages logged yet.</p>
                ) : (
                  <div className="space-y-2 max-h-[55vh] overflow-y-auto pr-1">
                    {leadDetail.messages.map((m) => (
                      <div
                        key={m.id}
                      ...
                      >
                        ...
                        >
                          <div className="flex items-center gap-2 text-[10px] uppercase tracking-wide opacity-70 mb-0.5">
                            <span>{m.direction === "in" ? "merchant" : "bot"}</span>
                            <span>·</span>
                            <span>{m.messageType}</span>
                            {m.state && <><span>·</span><span>{m.state}</span></>}
                          </div>
                          {m.body ? (
                          ...
                          {m.mediaUrl && (
                            <p className="text-[10px] opacity-70 mt-1 font-mono break-all">media: {m.mediaUrl}</p>
                          )}
      ...
      <Dialog open={!!emailDetail || emailDetailLoading} onOpenChange={(open) => { if (!open) setEmailDetail(null); }}>
        <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto">
          <DialogHeader><DialogTitle>Support Email</DialogTitle></DialogHeader>
          {emailDetailLoading || !emailDetail ? (
            <div className="p-8 text-center text-sm text-slate-400">Loading...</div>
          ) : (
            <div className="space-y-5">
              <div>
                <p className="text-xs font-semibold text-slate-400 uppercase tracking-wide mb-2">Headers</p>
                <div className="grid grid-cols-2 gap-3 text-sm">
                  <div>
                    <span className="text-slate-500">From</span>
                    <p className="font-medium text-slate-900">{emailDetail.fromName || "—"}</p>
                    <p className="text-xs text-slate-600 font-mono break-all">{emailDetail.fromAddress}</p>
                  </div>
                  <div>
                    <span className="text-slate-500">To</span>
                    <p className="text-slate-900 font-mono text-xs break-all">{emailDetail.toAddress}</p>
                  </div>
                  <div className="col-span-2">
                    <span className="text-slate-500">Subject</span>
                    <p className="text-slate-900">{emailDetail.subject || <span className="text-slate-400">—</span>}</p>
                  </div>
                  <div>
                    <span className="text-slate-500">Received</span>
                    <p className="text-slate-900">{new Date(emailDetail.receivedAt).toLocaleString()}</p>
                  </div>
                  <div>
                    <span className="text-slate-500">Language</span>
                    <p className="text-slate-900 uppercase">{emailDetail.language || "—"}</p>
                  </div>
                  <div>
                    <span className="text-slate-500">Action</span>
                    {emailDetail.classifiedAction === "auto_reply" ? (
                      <Badge variant="secondary" className="bg-teal-100 text-teal-700 mt-1">auto-reply drafted</Badge>
                    ) : (
                      <Badge variant="secondary" className="bg-slate-100 text-slate-700 mt-1">skipped — {emailDetail.skippedReason || "no reason"}</Badge>
                    )}
                  </div>
                  <div>
                    ...
                    <p className="text-slate-900">
                      {emailDetail.sentAt
                        ? new Date(emailDetail.sentAt).toLocaleString()
                        : <span className="text-slate-400">not sent (read-only phase)</span>}
                    </p>
                  </div>
                </div>
              </div>
              ...
              <div>
                <p className="text-xs font-semibold text-slate-400 uppercase tracking-wide mb-2">Inbound body</p>
                <pre className="text-sm text-slate-700 whitespace-pre-wrap break-words bg-slate-50 border border-slate-200 rounded-md p-3 max-h-[35vh] overflow-y-auto">{emailDetail.bodyText || "(empty)"}</pre>
              </div>

              {emailDetail.generatedReply && (
                <div>
                  <p className="text-xs font-semibold text-slate-400 uppercase tracking-wide mb-2">Drafted reply (would-be)</p>
                  <pre className="text-sm text-slate-900 whitespace-pre-wrap break-words bg-teal-50 border border-teal-200 rounded-md p-3 max-h-[35vh] overflow-y-auto">{emailDetail.generatedReply}</pre>
                </div>
              )}
            </div>
          )}
